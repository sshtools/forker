 ...
 */
final class ProcessKqueue extends BaseEventProcessor<NonBlockingOsxProcess> {
	...
	// Re-used in process() to avoid repeatedly allocating and destroying array
	// of events.
	private Kevent[] processEvents;
	private BlockingQueue<NonBlockingOsxProcess> closeQueue;
	private BlockingQueue<NonBlockingOsxProcess> wantsWrite;
	static {
		JAVA_PID = LibC.getpid();
	}

	ProcessKqueue(NonBlockingProcessFactory factory) {
		super(factory, 0);
		kqueue = LibKevent.kqueue();
		if (kqueue < 0) {
			throw new RuntimeException("Unable to create kqueue");
		}
		closeQueue = new ArrayBlockingQueue<NonBlockingOsxProcess>(512);
		wantsWrite = new ArrayBlockingQueue<NonBlockingOsxProcess>(512);
		processEvents = (Kevent[]) new Kevent().toArray(NUM_KEVENTS);
		// Listen for self-generated signal to tell processor to wake-up and
		// handling pending stdin requests
	...
	// ************************************************************************
	@Override
	public void registerProcess(NonBlockingOsxProcess process) {
		if (shutdown) {
			return;
		}
		...
		try {
			...
			// We don't use the processEvents array here, since this method is
			// not
			...
			// Listen for stdout and stderr data availability (events deleted
			// automatically when file descriptors closed)
			...
			// Listen for stdin data availability (initially disabled until user
			// wants read, deleted automatically when file descriptor closed)
			events[3].EV_SET(stdinFd, Kevent.EVFILT_WRITE, Kevent.EV_ADD | Kevent.EV_DISABLE | Kevent.EV_RECEIPT, 0, 0L,
					pidPointer);
	...
	private void registerEvents(Kevent[] keventArray, int numEvents) {
		...
		// to copy the fields of the structure to native memory (and read()
		// afterwards).
		...
		if (ret != numEvents) {
			...
			// On failure, at least one returned event will have a non-0 value
			// in the 'data' field.
	...
	@Override
	public void queueWrite(NonBlockingOsxProcess process) {
		if (shutdown) {
			return;
		}
		try {
	...
	@Override
	public void closeStdin(NonBlockingOsxProcess process) {
		try {
	...
	public boolean process() {
		...
		if (pidToProcessMap.isEmpty()) {
			// No processes registered. Wait up to LINGER_TIME_MS for a process
			// to show up
			// and signal one of its registered events.
			//
			// We know at least one event is registered on the kqueue
			// (EVFILT_SIGNAL),
			// so kevent() will block. (If no events are registered, kevent()
			// will not block
			// even if you pass it a timeout, unlike select()).
			timeout = new TimeSpec();
			// kevent() will return EINVAL if tv_nsec represents a value > 1
			// second,
			// so split the linger time over tv_sec and tv_nsec.
			timeout.tv_sec = factory.getLingerTimeMs() / 1000;
			timeout.tv_nsec = TimeUnit.MILLISECONDS.toNanos(factory.getLingerTimeMs() % 1000);
		} else {
			// At least one process is registered. There's no need to have a
			// timeout.
			timeout = null;
		}
	...
	private void processEvent(Kevent kevent) {
		...
		NonBlockingOsxProcess osxProcess = pidToProcessMap.get(udata);
		if (osxProcess == null) {
		...
		if (filter == Kevent.EVFILT_READ) // stdout/stderr data available to
											// read
		{
		...
		} else if (filter == Kevent.EVFILT_WRITE) // Room in stdin pipe
													// available to write
		{
			try {
				...
				if (ident == stdinFd) {
					...
					if (!userWantsMore) {
						...
						// We could use processEvents here and overwrite just
						// the first entry, but this probably doesn't happen
		...
		} else if ((kevent.fflags & Kevent.NOTE_EXIT) != 0) // process has
															// exited
															// System.gc()
		{
	...
	private void checkStdinCloses() {
		List<NonBlockingOsxProcess> processes = new ArrayList<NonBlockingOsxProcess>();
		// drainTo() is known to be atomic for ArrayBlockingQueue
		closeQueue.drainTo(processes);
		for (NonBlockingOsxProcess process : processes) {
	...
	private void checkWaitWrites() {
		...
		if (!processes.isEmpty()) {
			...
			// We don't use the processEvents array here, since this is invoked
			// from process()
			// and we would be overwriting the contents of that array while it's
			// being processed.
			Kevent[] kevents = (Kevent[]) new Kevent().toArray(processes.size());
			int numKevents = 0;
			for (int i = 0; i < processes.size(); i++) {
				NonBlockingOsxProcess process = processes.get(i);
				try {
	...
	private void cleanupProcess(NonBlockingOsxProcess osxProcess) {
		LibC.waitpid(osxProcess.getPID(), new IntByReference(), LibC.WNOHANG);
		// If this is the last process in the map, this thread will cleanly shut
		// down.
		pidToProcessMap.remove(osxProcess.getPID());
		Iterator<NonBlockingOsxProcess> iterator = wantsWrite.iterator();
